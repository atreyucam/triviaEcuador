...
// Define las rutas de las imágenes relativas a la carpeta public
const categoryImages = {
  costa: '../../../public/images/regionCosta.jpg',
  sierra: '../../../public/images/regionSierra.jpg',
  oriente: '../../../public/images/regionAmazonia.jpg'
//   galapagos: '/images/regionGalapagos.jpg'
}
...
export default function Questions () {
  ...
  useEffect(() => {
    const category = categories.find(cat => cat.name.toLowerCase() === questions[currentQuestion - 1]?.topic.toLowerCase())
    if (category) {
      document.body.style.backgroundColor = category.color
      const categoryKey = category.name.toLowerCase()
      if (categoryImages[categoryKey]) {
        setCategoryImage(categoryImages[categoryKey])
      } else {
        setCategoryImage('')
      }
    }
    ...
    <>
      ...
      {categoryImage && (
        <div className="absolute top-4 left-1/2 transform -translate-x-1/2">
          <Image src={categoryImage} alt="Categoría" width={50} height={50} className="rounded-full" />
        </div>
      )}

      {queries.timemode && (
        <div className={`bg-white flex items-center justify-center w-14 aspect-square absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full text-2xl text-slate-900 font-medium ${time < 6 && win === undefined ? 'pulse_animation' : ''}`}>
          {time}
